...
function hasClass(lengthCell, className) {
    if (lengthCell.classList) return lengthCell.classList.contains(className)
    else return !!lengthCell.className.match(new RegExp('(\\s|^)' + className + '(\\s|$)'))
}

function addClass(lengthCell, className) {
    if (lengthCell.classList) lengthCell.classList.add(className)
    else if (!hasClass(lengthCell, className)) lengthCell.className += " " + className
}

function removeClass(lengthCell, className) {
    if (lengthCell.classList) lengthCell.classList.remove(className)
...
function total() {
    ...
    alert('You have exercised for a total of ' + sum + ' minutes!');
}